...
def pptx_to_pdf(pptx_path, output_pdf):
    try:
        ...
        for i, slide in enumerate(prs.slides):
            img_path = os.path.join(UPLOAD_FOLDER, f"slide_{i}.png")
...
def convert_file():
    ...
    if ext == ".pdf" and convert_type == "docx":
        output_path += ".docx"
        msg = pdf_to_docx(input_path, output_path)
    elif ext == ".docx" and convert_type == "pdf":
        output_path += ".pdf"
        msg = docx_to_pdf(input_path, output_path)
    elif ext == ".pptx" and convert_type == "pdf":
        output_path += ".pdf"
        msg = pptx_to_pdf(input_path, output_path)
    elif ext == ".pdf" and convert_type == "pptx":
        output_path += ".pptx"
